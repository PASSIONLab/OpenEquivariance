...
mlir.register_lowering(
    tp_fwd_p, mlir.lower_fun(tp_fwd_impl, multiple_results=False), platform="cuda"
)
mlir.register_lowering(
    tp_fwd_p, mlir.lower_fun(tp_fwd_impl, multiple_results=False), platform="rocm"
)
...
mlir.register_lowering(
    tp_bwd_p, mlir.lower_fun(tp_bwd_impl, multiple_results=True), platform="cuda"
)
mlir.register_lowering(
    tp_bwd_p, mlir.lower_fun(tp_bwd_impl, multiple_results=True), platform="rocm"
)
...
mlir.register_lowering(
    tp_dbwd_p, mlir.lower_fun(tp_dbwd_impl, multiple_results=True), platform="cuda"
)
mlir.register_lowering(
    tp_dbwd_p, mlir.lower_fun(tp_dbwd_impl, multiple_results=True), platform="rocm"
)
...
def tp_fwd_jvp_rule(primals, tangents, *, L3_dim, kernel, hash):
    ...
    out_tangent = tp_fwd_jvp_p.bind(
        X, Y, W, dX, dY, dW, L3_dim=L3_dim, kernel=kernel, hash=hash
    )
...
def tp_bwd_jvp_transpose(ct, X, Y, W, dZ, tX, tY, tW, tdZ, *, kernel, hash):
    ...
    g_X, g_Y, g_W, g_dZ = tp_dbwd_p.bind(*tensors_clean, kernel=kernel, hash=hash)
...
def tp_bwd_jvp_rule(primals, tangents, *, kernel, hash):
    ...
    out_tangent = tp_bwd_jvp_p.bind(
        X, Y, W, dZ, tX, tY, tW, tdZ, kernel=kernel, hash=hash
    )
...
def tp_dbwd_jvp_rule(primals, tangents, *, kernel, hash):
    ...
    def func(x, y, w, dz, ddx, ddy, ddw):
        return tp_dbwd_slow(
            x, y, w, dz, ddx, ddy, ddw, L3_dim=L3_dim, kernel=kernel, hash=hash
        )
