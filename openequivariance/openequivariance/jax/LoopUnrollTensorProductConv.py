...
class LoopUnrollTensorProductConv(LoopUnrollConv):
    r"""Apply the established loop-unroll convolution with JAX.
    ...
    def forward(
        ...
        *,
        ...
        row_ptr: Optional[jax.numpy.ndarray] = None,
    ) -> jax.numpy.ndarray:
        """Apply the loop-unroll convolution."""
        if not isinstance(indices_are_sorted, bool):
            raise TypeError("indices_are_sorted must be a Python bool")
        del row_ptr
        # Preserve rows, cols, and sender_perm exactly. Changing endpoints here
        # would invalidate a deterministic sender permutation.
...
__all__ = ["LoopUnrollTensorProductConv"]
