...
class FlashTPConv(ConvolutionBase):
    def __init__(self, config, *, idx_dtype=np.int64, torch_op=True):
        ...
        instructions = [
            (inst.i_in1, inst.i_in2, inst.i_out, inst.connection_mode, inst.has_weight, inst.path_weight)
            for inst in config.instructions
        ]

        self.internal = uvu_TP(config.irreps_in1, 
                config.irreps_in2, 
                config.irreps_out, 
                instructions, 
                device="cuda", 
    ...
    def forward(self, L1_in, L2_in, weights, rows, cols, transpose_perm=None):
        return self.internal(L1_in, L2_in, weights, rows.to(torch.int), cols.to(torch.int))
